...
func (s *databaseStore) GetAllExpenses(startDate, endDate *time.Time) ([]Expense, error) {
	var query string
	var args []interface{}
	...
	if startDate != nil && endDate != nil {
		...
		query = `SELECT id, recurring_id, name, category, amount, date, tags FROM expenses WHERE date >= $1 ORDER BY date DESC`
